...
async function verify(): Promise<void> {
	loadingSpinner.show();
	try {
		const response: SignedInUser = await service.verify(props.uuid);
		state.value = State.Success;
		store.setUserInfo(response);
		loadingSpinner.hide();
		toast.success(i18n.t('core.account.verification.messages.success'));
	} catch (error) {
		loadingSpinner.hide();
		switch (error?.response?.status) {
			case 400:
				state.value = State.AlreadyVerified;
				toast.error(i18n.t('core.account.verification.messages.alreadyVerified'));
				break;
			case 403:
				state.value = State.Banned;
				toast.error(i18n.t('core.account.verification.messages.banned'));
				break;
			case 404:
				state.value = State.NotFound;
				toast.error(i18n.t('core.account.verification.messages.notFound'));
				break;
			case 410:
				state.value = State.Expired;
				toast.error(i18n.t('core.account.verification.messages.expired'));
				break;
			default:
				state.value = State.Error;
				toast.error(i18n.t('core.account.verification.messages.error'));
				break;
		}
	}
}
