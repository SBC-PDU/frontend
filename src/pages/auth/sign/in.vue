...
async function onSubmit(): Promise<void> {
	...
	try {
		...
		loadingSpinner.hide();
		toast.success(i18n.t('core.sign.in.messages.success'));
	} catch (error) {
		loadingSpinner.hide();
		if (error instanceof AxiosError) {
			if (error.response?.status === 400) {
				...
				if (message.message === 'Invalid credentials') {
					toast.error(i18n.t('core.sign.in.messages.invalidCredentials'));
					return;
				}
				if (message.message === 'Incorrect 2FA code') {
					toast.error(i18n.t('core.user.totp.messages.incorrectCode'));
					return;
				}
				...
				toast.error(i18n.t('core.sign.in.messages.accountBlocked'));
				return;
			}
			toast.error(i18n.t('core.sign.in.messages.error'));
		}
	}
}
